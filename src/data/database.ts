...
// Albums (met subalbums per wedstrijd)
...
// Photos (using placeholder images from Unsplash)
export const photos: Photo[] = [
  // Atletiek - BK Veldlopen
  {
    id: 1,
    title: 'Sprintfinish BK Veldlopen',
    description: 'De beslissende meters van de U23 categorie met een spannende sprint.',
  ...
  },
  {
    ...
    description: 'Een loper die zich door de zware modder vecht tijdens het BK.',
  ...
  // Atletiek - Dwars door Vlaanderen
  {
    id: 5,
    title: 'Start Dwars door Vlaanderen',
    description: 'Het startschot voor de 10km straatloop met honderden deelnemers.',
  ...
  },
  {
    ...
    description: 'Enthousiaste supporters langs de kant moedigen de lopers aan.',
    imageUrl: 'https://images.unsplash.com/photo-1513593771513-7b58b6c4af38?w=800',
    dateTaken: '2026-01-20',
    location: 'Roeselare, België',
    camera: {
  ...
  // Atletiek - Memorial Van Damme
  {
    id: 9,
    title: '100m Finale',
    description: 'De explosieve start van de 100 meter finale op de Diamond League.',
    imageUrl: 'https://images.unsplash.com/photo-1461896836934-ffe607ba8211?w=800',
  ...
  },
  {
    ...
    imageUrl: 'https://images.unsplash.com/photo-1593786481097-080e77c6ce04?w=800',
    dateTaken: '2025-11-18',
    location: 'Gent, België',
    camera: {
      model: 'Canon EOS R3',
  ...
  // Jiu-Jitsu
  {
    id: 17,
    title: 'Worpgreep',
    description: 'Het moment waarop een judoka zijn tegenstander optilt voor een worp.',
  ...
  },
  {
    ...
    imageUrl: 'https://images.unsplash.com/photo-1583473848882-f9a5bc7fd2ee?w=800',
    dateTaken: '2024-03-12',
    location: 'Antwerpen, België',
    camera: {
      model: 'Sony A1',
...
// Organisaties waarmee Studio Storm heeft samengewerkt
export const organizations: Organization[] = [
  ...
  },
  {
    id: 2,
    name: 'Agones Media',
    website: 'https://agones.be',
    description: 'Sportmediaorganisatie',
  },
  {
    id: 3,
    name: 'Runnerslab Athletics Team',
    website: 'https://runnerslab.be',
    description: 'Atletiekteam en hardloopspeciaalzaak',
  },
  {
    id: 4,
    name: 'VAL - Vlaamse Atletiekliga',
    description: 'Vlaamse atletiekfederatie',
  },
];
...
export const getSubAlbums = (parentId: number): Album[] => {
  return albums.filter((album) => album.parentId === parentId);
};

export const getMainAlbums = (): Album[] => {
  return albums.filter((album) => !album.parentId);
};

export const getPhotoCountForAlbum = (albumId: number): number => {
  const album = getAlbumById(albumId);
  if (!album) return 0;
  
  // Als het een hoofdalbum is, tel alle foto's van subalbums en direct gekoppelde foto's
  if (!album.parentId) {
    const subAlbums = getSubAlbums(albumId);
    const subAlbumPhotos = subAlbums.reduce((count, subAlbum) => {
      return count + photos.filter(p => p.albumId === subAlbum.id).length;
    }, 0);
    const directPhotos = photos.filter(p => p.albumId === albumId).length;
    return directPhotos + subAlbumPhotos;
  }
